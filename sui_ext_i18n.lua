...
local function detectExtDir()
    local src = debug.getinfo(1, "S").source
    if type(src) == "string" then
        local dir = src:match("^@(.+)/[^/]+$")
        if dir and dir:find("simpleui_ext%.koplugin", 1, true) then
...
local _dir = detectExtDir()

-- ---------------------------------------------------------------------------
-- .po parser — minimal implementation supporting the subset generated by
-- simpleui_ext.pot (identical 1:1 copy of the base plugin's parser logic)
-- ---------------------------------------------------------------------------
